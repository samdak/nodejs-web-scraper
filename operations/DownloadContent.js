...
class DownloadContent extends HttpOperation {
  ...
  constructor(querySelector, config = {}) {
    ...
    for (let prop in config) {
      if (this.overridableProps.includes(prop)) this.config[prop] = config[prop];
    }

    this.directoryVerified = false;

    this.alternativeSrc = config.alternativeSrc || ['data-src', 'data-src-large', 'data-src-medium', 'data-src-small'];
  }
